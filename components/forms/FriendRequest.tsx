...
const FriendRequest = ({currentUserId,userId}: FriendRequestProps) => {
    ...
    return (
        ...
        <button className={` flex h-10 w-48 align-middle bg-light-2 justify-around p-2 text-black self-center rounded-md border-2 border-spacing-10 ${isFriend ? 'hover:border-[#E61D1D]' : isRequested ? 'hover:border-[#9C9F9B]' : 'hover:border-[#25DE00]'}`}
        onClick={handleClick}>
            ...
            {isFriend ? 'Unfriend' : isRequested ? 'Cancel Request' : 'Add Friend'}
        </button>
